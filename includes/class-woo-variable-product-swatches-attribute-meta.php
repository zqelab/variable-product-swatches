 ...
 */
class Woo_Variable_Product_Swatches_Attribute_Meta {
    ...
    public function __construct($taxonomy, $fields = []) {
        ...
        add_action( 'delete_'.$this->taxonomy,  array( $this, 'delete_term' ), 5, 4);
    ...
    public function edit_term_columns( $columns ) {
            ...
            $new_columns['woo-variable-product-swatches-meta'] = '';
    ...
    public function manage_term_custom_column( $out, $column, $term_id ) {
        ...
        if( $attribute->type == 'color' ){
            ...
            printf('<div class="woo-variable-product-swatches-meta-preview" style="background-color:%s;"></div>', 
                esc_attr( $value ) 
            );
        }
        ...
        if( $attribute->type =='image' ){
            ...
            if ( is_array( $image ) ) {
                printf( 
                    '<img src="%s" alt="" width="%d" height="%d" class="woo-variable-product-swatches-meta-preview" />', 
                    esc_url( $image[0] ), 
                    $image[1], 
                    $image[2] 
                );
        ...
        foreach ($this->fields as $key => $field) {
            ...
            switch ($field['type']) {
                ...
                case 'image':
                    ob_start();
                    ?>
                    <div class="woo-variable-product-swatches-image-field-wrapper">
