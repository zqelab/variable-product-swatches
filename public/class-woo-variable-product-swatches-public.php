 ...
 * @subpackage Woo_Variable_Product_Swatches/public
 */
 ...
 */
class Woo_Variable_Product_Swatches_Public {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin->name, plugin_dir_url(__FILE__) . 'css/woo-variable-product-swatches-public.css', array(), $this->plugin->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin->name, plugin_dir_url(__FILE__) . 'js/woo-variable-product-swatches-public.js', array('jquery', 'wc-add-to-cart', 'wc-add-to-cart-variation'), $this->plugin->version, true );
		
		wp_localize_script( $this->plugin->name, '_WVPS', array(
			'option' => get_option( 'testx' ),
		) );
		...
		<style type="text/css">
			ul.swatches-items-wrapper>li.swatch-item.swatch-item-image .swatch-item-span {
		        width  : <?php echo $image_swatch_width ?>px;
		        height : <?php echo $image_swatch_height ?>px;
		    }
		    ul.swatches-items-wrapper>li.swatch-item.swatch-item-color .swatch-item-span {
		        width  : <?php echo $color_swatch_width ?>px;
		        height : <?php echo $color_swatch_height ?>px;
		    }
		</style>
		<?php
		$css = ob_get_clean();
	...
	public function body_class($classes) {
		...
		if ( $show_selected_attribute ) {
			$classes[] = 'woo-variable-product-swatches-show-selected-attribute';
		}

		if ( $clickable_disabled_variation ) {
			$classes[] = 'woo-variable-product-swatches-clickable-disabled-attribute';
		}

		if ( $stockcount ) {
			$classes[] = 'woo-variable-product-swatches-stockcount';
		}

		if ( $attribute_behavior ) {
			$classes[] = 'woo-variable-product-swatches-' . $attribute_behavior;
		}
	...
	public function woocommerce_dropdown_variation_attribute_options_html_filter($html, $args) {
		...
		if (apply_filters('woo_variable_product_swatches_no_individual_settings', true, $args, $image_default, $button_default)) {
	...
	public function variation_attribute_options($args) {
		...
		if ($type === 'select') {
			...
			echo '<select id="' . esc_attr($id) . '" name="' . esc_attr($name) . '" data-attribute_name="attribute_' . esc_attr(sanitize_title($attribute)) . '" data-show_option_none="' . ($show_option_none ? 'yes' : 'no') . '" class="' . esc_attr($class) . ' woo-variable-product-swatches-raw-select woo-variable-product-swatches-raw-type-' . esc_attr($type) . '" style="display:none;" >';
		}
